...
class DxEntityFinder:
    def __init__(self, ctyfilepath=None, dxccjsonfilepath=None):
        ...
        if ctyfilepath is not None:
            ...
            if dxccjsonfilepath is not None:
                with open(dxccjsonfilepath, "r") as f:
                    ...
                    self.dxcc_to_country_map['United States'] = self.dxcc_to_country_map['United States of America']
                    self.dxcc_to_country_map['Fed. Rep. of Germany'] = self.dxcc_to_country_map['Germany']
                    self.dxcc_to_country_map['Pitcairn Island'] = self.dxcc_to_country_map['Pitcairn Islands']
                    self.dxcc_to_country_map['Curacao'] = self.dxcc_to_country_map['Curaçao']
                    self.dxcc_to_country_map['Wallis & Futuna Islands'] = self.dxcc_to_country_map['Wallis and Futuna Islands']
                    self.dxcc_to_country_map['Republic of Korea'] = self.dxcc_to_country_map['South Korea']
                    self.dxcc_to_country_map['Western Kiribati'] = self.dxcc_to_country_map['Gilbert Islands']
                    self.dxcc_to_country_map['Eastern Kiribati'] = self.dxcc_to_country_map['Line Islands']
                    self.dxcc_to_country_map['Central Kiribati'] = self.dxcc_to_country_map['Phoenix Islands']
                    self.dxcc_to_country_map['Antigua & Barbuda'] = self.dxcc_to_country_map['Antigua and Barbuda']
                    self.dxcc_to_country_map['Saba & St. Eustatius'] = self.dxcc_to_country_map['Saba and Sint Eustatius']
                    self.dxcc_to_country_map['Madeira Islands'] = self.dxcc_to_country_map['Madeira']
                    self.dxcc_to_country_map['Trinidad & Tobago'] = self.dxcc_to_country_map['Trinidad and Tobago']
                    self.dxcc_to_country_map['St. Lucia'] = self.dxcc_to_country_map['Saint Lucia']
                    self.dxcc_to_country_map['Vietnam'] = self.dxcc_to_country_map['Viet Nam']
                    self.dxcc_to_country_map['St. Vincent'] = self.dxcc_to_country_map['Saint Vincent and the Grenadines']
                    self.dxcc_to_country_map['Slovak Republic'] = self.dxcc_to_country_map['Slovakia']
                    self.dxcc_to_country_map['Republic of Kosovo'] = self.dxcc_to_country_map['Kosovo']
                    self.dxcc_to_country_map['Sicily (not DXCC)'] = self.dxcc_to_country_map['Italy']
                    self.dxcc_to_country_map['Tristan da Cunha & Gough'] = self.dxcc_to_country_map['Tristan da Cunha and Gough Islands']
                    self.dxcc_to_country_map['Timor - Leste'] = self.dxcc_to_country_map['Timor-Leste']
            else:
                raise Exception(
    ...
    def find_one_dx_entity(self, callsign):
        ...
        if result:
            ...
            try:
                ...
                print(f"Unable to find DXCC.json mapping for <{result[0]}>, rendering country code as XX")
                ret["country_code"] = "XX"
        return ret
    ...
    def do_find_dx_entities(self):
        with engine.connect() as conn:
            ...
            stmt = select(qso_table.c.callsign).except_(select(dx_entity_table.c.callsign))
            
            callsign_entity_structs = [self.find_one_dx_entity(row.callsign) for row in conn.execute(stmt)]
...
def calculate_distances():
    row_updates = []
    with engine.connect() as conn:
        stmt = select(qso_table.c.callsign, qso_table.c.station_loc, qso_table.c.callsign_loc).where(
        ...
        for row in conn.execute(stmt):
            dist_and_bearing = grid_distance(Grid(row.station_loc), Grid(row.callsign_loc))
            if dist_and_bearing and dist_and_bearing[0]:
                row_updates.append({'b_callsign': row.callsign, 'enr_distance': dist_and_bearing[0]})
...
if __name__ == "__main__":
    ...
    dx_finder = DxEntityFinder(ctyfilepath = "datasources/cty.dat", dxccjsonfilepath = "datasources/dxcc.json")
